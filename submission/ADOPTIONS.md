...
| **`tee:agent-connect` (T3's own commerce rails)** | **Not implemented — and we owe the platform a correction.** While probing discovery we found the node already serves `tee:agent-connect@1.4.0`: `commerce-intent-create`, `commerce-quote`, `commerce-intent-confirm`, `commerce-intent-cancel`, `commerce-history-get`, `commerce-webhook-apply`. It is documented nowhere and has no SDK helper (bug #26). It is the natural home for this gate — a mandate deciding whether an intent may be confirmed — but its inputs have to be brute-forced from parse errors (bug #25), and building against a surface we had to reverse-engineer this week would be guesswork, not an adoption. Filed, described, and left honestly unbuilt. |
| **`tee:vc` OpenID4VP holder stack** | **Not implemented**, same reason. `tee:vc@2.6.0` carries `submit-vp` (a real DCQL/OpenID4VP request — it answers `unsatisfied: query could not be satisfied with available credentials`), `my-presentations`, and `issue-credential`. This is the holder-side derive our June bug #3 said did not ship: it does, on the node, undocumented. We cannot use it yet because issuing into it needs issuer metadata (`keys.generic_api metadata is required`) with no documented way to register. This is the most promising route to closing our biggest correctness gap — see the `vp.verify` row. |
