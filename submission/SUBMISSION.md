...
| **Signed card + did:web** | The published card is signed (EdDSA JWS over its canonical form) with the agent's key, and `/.well-known/did.json` publishes that key as `did:web:gatekeeper-evidence.vercel.app`. A peer verifies the card by DID resolution — nothing shared in advance |
| **A2A streaming, and an honest replay window** | `SendStreamingMessage` streams task → working → artifact → completed as SSE, locally and hosted. The hosted door bounds signature age to 120 s because its nonce ledger is per instance — said out loud, not hidden |
...
| Test | 47 Rust + 223 Node + 42 Playwright + 3 axe + 5 page-interaction, all green — `node verify.mjs` counts them itself |
...
the *real* Rust decision function, then axe-core over both pages. 321 checks, no API key, no network, no credits.
...
**`node verify.mjs` reports its own total — 321 offline checks** at the time of
writing (47 Rust, 223 Node, 42 Playwright end-to-end, 3 axe-core, 5 page interaction), plus the live-site and
submission-artifact suites (27 live, including signed and streamed calls to the hosted doors). The number comes from the runners rather than from
...
| 07 | `07-tests.png` | `node verify.mjs` — 321 checks, no key, no credits |
...
| **Also live** | the evidence site, the A2A agent card, the ERC-8004 registration file, the Web Bot Auth key directory and the W3C revocation status list — every live assertion in the suite passes (27/27) |
