...
| Offline unit tests | `cd agent && npm test` | 223/223 pass across 19 files (issue/verify, tamper, SD, edge cases, A2A client + server, MCP over stdio and HTTP, Web Bot Auth + key directory + Cloudflare interop + freshness window, revocation, credential binding, audit, ERC-8004, status list, x402, discovery, the transpiled wasm component, the registry manifest, the hosted bundle, the signed card) |
| TEE contract unit tests | `cd gate-contract && cargo test` | 47 unit + 1 doc test pass |
...
| Contract deploy | `cd agent && npm run setup` | registered — **live is `gate@0.10.0` = `contract_id 749`** (2026-08-27). Earlier: `0.7.0` = `479`, `0.6.0` = `175` |
...
| Mandate rules, offline | `cd qa-console && node --test e2e.test.mjs` | 42 Playwright tests over the contract's real Rust `decide()` — happy paths, wrong paths, API abuse, x402 through the mandate, signed A2A + MCP by hand-written JSON-RPC, and the server-gone error state |
| Evidence site | `cd qa-console && node --test site.test.mjs` | 27 tests incl. every screenshot renders (naturalWidth ≠ 0), a Web Bot Auth key round trip over the public internet, did:web resolution of the signed card, and the hosted `/api/a2a` + `/api/mcp` doors refusing unsigned calls and answering signed ones |
...
> **Deployment state (30 Aug 2026).** Source and network agree: **v0.10.0** is
> live as **`contract_id 749`**, registered 2026-08-27 once the account was
> topped up, and promoted only after `npm run probe` registered it under a
> throwaway tail and invoked it. It carries everything v0.8.0 and v0.9.0 added
> while the balance was zero (bugs #16, #17): `allowed_issuers`, the
> per-counterparty sub-limits, the enclave-held credential, the credential/action
> binding, idempotent dispatch and `http-with-placeholders`. The rows above were
> re-run against it; `node verify.mjs` prints **321** and
> `cd qa-console && npm run test:site` prints **27**, both on 30 Aug 2026.
...
**29 in total**, the full ledger being [`BUGS.md`](BUGS.md). The eleven found in August are written up in
